...
namespace Planning.NSS
{
    ...
    {
        public SPL_V_PL0_TypeTache(ITBfrStorageAccess storageAccess) : base(storageAccess) { }

        public override bool UniquenessCheck(TBfrData_List data_List, Object iHMObject, TBfrError_List errors)
        {
            var vData_List = (DPL_V_PL0_TypeTache)data_List;
            return new SDBPL_V_DB0_TypeTache(IStorageAccess).UniquenessCheck(iHMObject, errors, vData_List.DId, vData_List.DLibelle);
        }

        public override bool NotUsedCheck(long id, bool forPhysicalDelete, object iHMObject, TBfrError_List errors)
           => new SDBPL_V_DB0_TypeTache(IStorageAccess).NotUsedCheck(id, forPhysicalDelete, iHMObject, errors);

        public override int Merge(long oldId, long newId)
           => new SDBPL_V_DB0_TypeTache(IStorageAccess).Merge(oldId, newId);

        public override int Delete(long id)
           => new SDBPL_V_DB0_TypeTache(IStorageAccess).Delete(id);

        public override int Neutralize(long id)
           => new SDBPL_V_DB0_TypeTache(IStorageAccess).Neutralize(id);
